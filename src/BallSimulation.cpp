...
void BallSimulation::UpdateBalls(const sf::Vector2u &windowSize, float deltaTime) const {
    ...
    for (auto& [hashPosition1, bucket]: spatialHash->buckets) {
        // collisions within own bucket
        ...
        const auto neighbouringBuckets = spatialHash->GetNeighboringBuckets(hashPosition1);
        for (size_t b = 0; b < neighbouringBuckets.size(); ++b) {
            if (neighbouringBuckets[b] == nullptr)
                continue;
            const auto bucket2 = *neighbouringBuckets[b];

            // collisions with neighbouring buckets
            for (size_t i = 0; i < bucket.size(); ++i) {
                for (size_t j = 0; j < bucket2.size(); ++j) {
                    HandleCollision(bucket[i], bucket2[j]);
                }
            }
        }
    }
...
void BallSimulation::HandleCollision(const std::shared_ptr<Ball>& ball1, const std::shared_ptr<Ball>& ball2) {
    if (ball1 == ball2 || ball1 == nullptr || ball2 == nullptr)
        return;
